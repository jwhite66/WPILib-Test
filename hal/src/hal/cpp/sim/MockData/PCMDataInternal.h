...
class PCMData {
 public:
  int32_t RegisterSolenoidInitializedCallback(int32_t channel, HAL_NotifyCallback callback, void* param, HAL_Bool initialNotify);
  ...
  int32_t RegisterSolenoidOutputCallback(int32_t channel, HAL_NotifyCallback callback, void* param, HAL_Bool initialNotify);
 ...
 private:
  ...
  std::shared_ptr<NotifyListenerVector> m_solenoidInitializedCallbacks[kNumSolenoidChannels];
  std::atomic<HAL_Bool> m_solenoidOutput[kNumSolenoidChannels];
  std::shared_ptr<NotifyListenerVector> m_solenoidOutputCallbacks[kNumSolenoidChannels];
